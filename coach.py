...
class Coach():
    def __init__(self, opts):
        ...
        self.guidance = StableDiffusion(opts.gpu_ids[0], opts.fp16, opts.vram_O, opts.sd_version, opts.hf_key, opts.t_range, opts.n_src_views)
        for p in self.guidance.parameters():
            p.requires_grad = False
        
        self.clip = CLIP(opts.gpu_ids[0])
        ...
        for split in splits:
            if getattr(self.opts, f'data_{split}', None):  
                ...
                for data_opts in data_opts_list:
                    ...
                    cur_loader = DataLoader(cur_dataset, shuffle=(split == 'train'), num_workers=0,
    ...
    def build_networks(self):
        ...
        self.model = models_dict[self.opts.model](self.opts).to(self.opts.device)   # load IDNeRF to gpu
    ...
    def train_iteration(self, var):
        ...
        var_pred = self.model(var, guidance=self.guidance, clip=self.clip, mode="train")
    ...
    def validate_model(self, iter=None, is_sanity_check=False):
        ...
        for batch_idx, batch in enumerate(tqdm_loader):
            ...
            var = utils.move_to_device(var, self.opts.device)
            var_pred = self.model(var, self.guidance, self.clip, mode="val")
    ...
    def test_model(self, ep=None, send_log=True, save_images=True, leave_tqdm=False, is_sanity_check=False):
        ...
        for data_loader in self.test_loaders:
            ...
            for batch_idx, batch in enumerate(tqdm.tqdm(data_loader, desc=tqdm_desc, leave=leave_tqdm)):
                ...
                var = edict(batch)
                var = utils.move_to_device(var, self.opts.device)
                var = self.model(var, self.guidance, self.clip, mode="test")
