...
class GMFlow(nn.Module):
    def __init__(self,
        ...
        self.upsample_factor = upsample_factor  # 2
        self.attention_type = attention_type
        self.num_transformer_layers = num_transformer_layers    # 6
        self.feature_upsampler = feature_upsampler  # network
    ...
    def extract_feature(self, imgs):
        ...
        low_res_feature = features[0]
        for i in range(len(features)):
            feature = features[i]  # (b * n_views, n_feat_dim, h_i, w_i)
            feature = feature.reshape(b, int(feature.shape[0] / b), *feature.shape[-3:])  # (b, n_views, n_feat_dim, h_i, w_i)
            cur_feat0, cur_feat1 = [], []
        ...
        return feature0_list, feature1_list, low_res_feature
    ...
    def forward(self, imgs, attn_splits_list=None, layers_caps=None, keep_raw_feats=False, wo_cross_attn=False,
            ...
            feature0_list, feature1_list, low_res_feature = self.extract_feature(self.normalize_images(imgs))    
            
            # forward through the transformer structure
            all_scales = list(range(self.num_scales))   # [0]
            
            if len(all_scales) != len(attn_splits_list):    # [2]
                last_value = all_scales[-1]
                for _ in range(len(attn_splits_list) - len(all_scales)):
                    all_scales.append(last_value)

            if keep_raw_feats:  # True
            ...
            for att_idx, scale_idx in enumerate(all_scales):
                # print(att_idx, scale_idx)   # 0 0

                feature0, feature1 = feature0_list[scale_idx], feature1_list[scale_idx]
                attn_splits = attn_splits_list[att_idx] # 2
                layers_cap = layers_caps[att_idx]   # None
            ...
            results_dict.update({
                ...
                "low_res_feature": low_res_feature})
