...
def single_head_split_window_attention(q, k, v,
    ...
    b_new = b * num_splits * num_splits     # b*2*2,
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, source, target,
        ...
        # source, target: [B, L, C]
        if wo_cross_attn:
            assert not wo_self_attn, "must have at least one type of attention!!"
...
class FeatureTransformer(nn.Module):
    ...
    def forward(self, feature0, feature1,
                attn_num_splits=None,   # 2
                layers_cap=None,   # None
                wo_cross_attn=False,
                wo_self_attn=False, # false
                **kwargs,
                ):
        ...
        if layers_cap is None:
            layers_cap = len(self.layers)

        for layer_idx, layer in enumerate(self.layers):
            if layer_idx >= layers_cap:
                break
