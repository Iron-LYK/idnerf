...
from .latent_inference import latent_inference
import torch.nn.functional as F

class IDNeRF(torch.nn.Module):
    def __init__(self, opts):
        ...
        self.lambda_guidance = opts.lambda_guidance
        ...
        # define latent inference model
        self.latent_inference = latent_inference(opts.latent_inference.input_channel, opts.latent_inference.latent_dim)

        # define decoder
        self.nerf_dec = CondNeRF(opts).to(opts.device)

    def forward(self, batch, guidance, clip, mode=None, render_video=False, render_path_mode='interpolate'):
        ...
        tv_pose, ref_poses = self.extract_poses(batch) 

        latent_tv, input_ray_imgs = self.latent_inference(ref_images, ref_poses, tv_pose)    # 

        if mode=="train":
            with autocast():
                prompt_embedding_refimg = clip.get_img_embeds(ref_images.squeeze())
                prompt_embedding_refimg = prompt_embedding_refimg.unsqueeze(0)
                prompt_embedding_refimg = F.interpolate(prompt_embedding_refimg, size=(1024,), mode='linear', align_corners=True)
                
                prompt_embedding_input_ray_o = clip.get_img_embeds(input_ray_imgs[:,:3].squeeze())
                prompt_embedding_input_ray_o = prompt_embedding_input_ray_o.unsqueeze(0)
                prompt_embedding_input_ray_o = F.interpolate(prompt_embedding_input_ray_o, size=(1024,), mode='linear', align_corners=True)

                prompt_embedding_input_ray_d = clip.get_img_embeds(input_ray_imgs[:,3:].squeeze())
                prompt_embedding_input_ray_d = prompt_embedding_input_ray_d.unsqueeze(0)
                prompt_embedding_input_ray_d = F.interpolate(prompt_embedding_input_ray_d, size=(1024,), mode='linear', align_corners=True)

                prompt_embedding = torch.cat((prompt_embedding_refimg, prompt_embedding_input_ray_o, prompt_embedding_input_ray_d), dim=1)          

                uncond_embedding = guidance.get_text_embeds([""])
                prompt_embedding = torch.cat((uncond_embedding, prompt_embedding), dim=0)

                guidance.train_step(prompt_embedding, latent_tv, as_latent=True, guidance_scale=self.guidance_scale, grad_scale=self.lambda_guidance)
        ...
        if render_video:
            ...
            poses_paths = self.get_video_rendering_path(tv_pose, ref_poses, render_path_mode, self.opts.nerf.video_n_frames, batch)
        else:
            poses_paths = [tv_pose]

        # render images
        mode_rand_rays = getattr(self.opts.nerf, f'rand_rays_{mode}', 0)
        for frame_idx, cur_tv_pose in enumerate(tqdm(poses_paths, desc="rendering video frame...", leave=False) if render_video else poses_paths):
            if mode_rand_rays and mode in ["train", "test-optim"]:
                ...
                ret = self.render(self.opts, cur_tv_pose, ray_idx=batch.ray_idx, mode=mode,
                                  ref_poses=ref_poses, ref_images=ref_images, ref_feats_list=ref_feats_list, latent=latent_tv)  
            
            else:
                # render full image (process in slices)
                if mode_rand_rays: 
                    ret = self.render_by_slices(self.opts, cur_tv_pose, mode=mode,
                                                ref_poses=ref_poses, ref_images=ref_images, ref_feats_list=ref_feats_list, latent=latent_tv)
                else:
                    ret = self.render(self.opts, cur_tv_pose, mode=mode,
                                      ref_poses=ref_poses, ref_images=ref_images, ref_feats_list=ref_feats_list, latent=latent_tv)  
    ...
    def render(self, opt, tv_pose=None, ray_idx=None, mode=None,
               ref_poses=None, ref_images=None, ref_feats_list=None, latent=None):
        batch_size, _, _, img_h, img_w = ref_images.shape
        if tv_pose is None:
            raise Exception('Must provide tv_pose.')

        # casting ray with the given camera parameters, in cannonical world sapce
        center, ray = camera.get_center_and_ray(img_h, img_w, tv_pose['extrinsics'], intr=tv_pose['intrinsics'],
        ...
        depth_samples = self.sample_depth(opt, batch_size, num_rays=ray.shape[1], near_far=tv_pose['near_fars'],
        ...
        cond_info = self.query_cond_info(pts_3D, ref_poses, ref_images, ref_feats_list) # # (b, n_rays, n_samples, n_dim)
        if latent is not None:
            cond_info["latent"] = latent
    ...
    def render_by_slices(self, opt, tv_pose, mode=None,
                         ref_poses=None, ref_images=None, ref_feats_list=None, latent=None):
        assert ref_images is not None, "Must provide the reference images for IDNeRF."
        ...
        for c in tqdm(range(0, img_h*img_w, mode_rand_rays), desc=f"slicing per [{mode_rand_rays}] rays...", leave=False):
            ...
            ret = self.render(opt, tv_pose, ray_idx=ray_idx, mode=mode,
                              ref_poses=ref_poses, ref_images=ref_images, ref_feats_list=ref_feats_list, latent=latent)  # [B,R,3],[B,R,1]
            for k in ret:
    ...
    def query_cond_info(self, point_samples, ref_poses, ref_images, ref_feats_list):
        ...
        views_projected_feature = [[] for _ in range(n_views)]
        views_cond_feature = [[] for _ in range(n_views)]
        for i in range(n_views):
            projected_feature_0 = feat_data_list[0][i]
            projected_feature_1 = feat_data_list[1][i]
            projected_feature = torch.cat((projected_feature_0, projected_feature_1),dim=1)
            views_projected_feature[i] = projected_feature 
            views_cond_feature[i] = torch.cat((projected_feature,color_data[i],mask_data[i]),dim=1)
        views_projected_feature = torch.cat(views_projected_feature,dim=1)
        all_data['view_cond_feat'] = views_cond_feature  
        ...
        all_data['matching_info'] = torch.cat([all_data['feat_info'], all_data['color_info'], all_data['mask_info']], dim=-1)

        return all_data

    def get_video_rendering_path(self, tv_pose, ref_poses, mode, n_frames=30, batch=None):
        # loop over batch
        poses_paths = []
        for batch_idx, cur_src_poses in enumerate(ref_poses['extrinsics']):
            if mode == 'interpolate':
                ...
                cur_near_far = tv_pose['near_fars'][batch_idx].detach().cpu().numpy().tolist()
        ...
        for frame_idx in range(n_frames):
            updated_tv_poses.append(dict(extrinsics=poses_paths[:, frame_idx],
                                          intrinsics=tv_pose['intrinsics'].clone().detach(),
                                          near_fars=tv_pose['near_fars'].clone().detach()))

        return updated_tv_poses
